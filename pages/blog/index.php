    ...
    <?php while ($blog->fetch()) : ?>
      ...
      <article class="rounded-xl bg-purple-600 p-3 shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300 ">
        ...
        <div class="relative flex items-end overflow-hidden rounded-xl">
          <img src="<?= BASE_PATH ?>assets/images/shows/<?= $imgPath ?>" alt="<?= $showName ?>" />
        </div>
            ...
            <div class="mt-3 flex items-end justify-between">
              <?php if ($published == 1) : ?>
                <div class="flex items-center space-x-1.5 rounded-lg bg-red-500 px-4 py-1.5 text-white duration-100 hover:bg-red-600">
                  <button onclick="window.location.href='<?= BASE_PATH ?>account/dashboard/admin/config/unpublishBlog.php?bid=<?= $blogID ?>';" class="text-sm">UNPUBLISH</button>
                </div>
              <?php else : ?>

                <div class="flex items-center space-x-1.5 rounded-lg bg-green-500 px-4 py-1.5 text-white duration-100 hover:bg-red-600">
                  <button onclick="window.location.href='<?= BASE_PATH ?>account/dashboard/admin/config/publishBlog.php?bid=<?= $blogID ?>';" class="text-sm">PUBLISH</button>
                </div>
              <?php endif ?>
            </div>
          <?php endif ?>
